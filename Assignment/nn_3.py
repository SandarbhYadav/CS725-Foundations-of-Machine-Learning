...
class PCA_custom:
    ...
    def fit(self, X):
        ...
        with open('features.csv', 'w') as f:
            for k in indexs[:self.principalFeatures]:
                f.write(feature_dict[k + 2])
                f.write('\n')
        f.close()
...
def train(
    ...
    print('MSE on dev data: {:.5f}'.format(dev_loss))
...
# pred_df.to_csv("22D1594_val.csv", index=False)
...
def main():
    ...
    part_3_df = pd.DataFrame(part_3_dict)
    part_3_df.to_csv("part_3.csv", index=False)
